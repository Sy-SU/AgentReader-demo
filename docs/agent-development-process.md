...
- 没有固定的搜索/检索质量数据集、指标或 Prompt 回归评测；
...
- **离线证据**：2026-09-12 当前完整回归运行 96 项测试，93 项通过，3 项真实
  联网测试按设计
  跳过。
...
- **离线证据**：当前完整回归 96 项测试中 93 项通过，3 项联网测试按设计跳过；
  编译、依赖检查和补丁格式检查通过。
...
- **下一小步**：建立固定问题和期望页码，计算 Recall@K、MRR 和无答案查询行为，
  同时记录首次索引与缓存命中的时延、磁盘和 Context 大小。
